...
function fdxReadiness() {
  ...
  const consentUiEnabled = boolEnv('FDX_CONSENT_UI_ENABLED', true);
  const openApiPublished = boolEnv('FDX_OPENAPI_PUBLISHED', true);
  const registryLinked = boolEnv('FDX_REGISTRY_LINKED', false);
  const auditLoggingEnabled = boolEnv('AUDIT_LOGGING_ENABLED', true);
  ...
  return {
    ...
    data_domains: {
      ...
      accounts: boolEnv('FDX_ACCOUNTS_API_ENABLED', true) ? 'implemented' : 'planned',
      transactions: boolEnv('FDX_TRANSACTIONS_API_ENABLED', true) ? 'implemented' : 'planned'
    },
    rails: railStatuses()
  };
}

...
class PlaceSuggestion {
  ...
  factory PlaceSuggestion.fromJson(Map<String, dynamic> json) {
    ...
    final name =
        json['name'] as String? ?? json['city_name'] as String? ?? 'Lieu';
    final iata = json['iata_code'] as String? ?? '';
    final city = json['city_name'] as String? ?? '';
    final country =
        json['iata_country_code'] as String? ??
        json['country_code'] as String? ??
        '';
    final label = [
      name,
      if (city.isNotEmpty && city != name) city,
      if (iata.isNotEmpty) iata,
      if (country.isNotEmpty) country,
    ].where((value) => value.isNotEmpty).join(' · ');
    return PlaceSuggestion(
      id: json['id'] as String? ?? '$type:$iata:$name',
      label: label,
      iataCode: iata,
      countryCode: country,
    );
...
class FlightOffer {
  ...
  factory FlightOffer.fromJson(Map<String, dynamic> json) {
    final slices = (json['slices'] as List? ?? const [])
        .whereType<Map>()
        .toList();
    final firstSlice = slices.isNotEmpty
        ? Map<String, dynamic>.from(slices.first)
    ...
    final summary = json['summary'] is Map
        ...
        : const <String, dynamic>{};
    final carrier =
        summary['primary_carrier'] as String? ??
        ((firstSegment['marketing_carrier'] as Map?)?['name'] as String?) ??
        'Compagnie aerienne';
    final departure = DateTime.tryParse(
      firstSegment['departing_at'] as String? ?? '',
    );
    final arrival = DateTime.tryParse(
      lastSegment['arriving_at'] as String? ?? '',
    );
    final duration = _duration(firstSlice['duration'] as String? ?? '');
    final origin =
        (firstSegment['origin'] as Map?)?['iata_code'] as String? ?? '';
    final destination =
        (lastSegment['destination'] as Map?)?['iata_code'] as String? ?? '';
    return FlightOffer(
      ...
      duration: duration,
      stops:
          (summary['connections'] as num?)?.toInt() ??
          (segments.length - 1).clamp(0, 99).toInt(),
  ...
  int get sortMinorUnits {
    ...
    final fraction = parts.length > 1
        ? int.tryParse('${parts[1]}00'.substring(0, 2)) ?? 0
        : 0;
...
class RailCapability {
  ...
  factory RailCapability.fromJson(Map<String, dynamic> json) => RailCapability(
    ...
    reason:
        ...
        'Disponibilite non confirmee par le serveur.',
    provider:
        json['provider'] as String? ?? json['rail'] as String? ?? 'Provider',
...
class PricingSnapshot {
  const PricingSnapshot({
    ...
    required this.providerFare,
    required this.fixedTicketingFee,
    required this.railFee,
    required this.total,
    required this.currency,
...
class PaygateApi {
  ...
  Future<List<PlaceSuggestion>> places(String query) async {
    final response = await _get('/api/duffel/places', {
      'query': query,
      'limit': '8',
    });
    ...
    return (data['data'] as List? ?? const [])
        ...
        .where((place) => place.iataCode.isNotEmpty)
        .toList();
  }
  ...
  Future<List<FlightOffer>> searchFlights({
    ...
    return (payload['offers'] as List? ?? const [])
        .whereType<Map>()
        .map((item) => FlightOffer.fromJson(Map<String, dynamic>.from(item)))
        .toList();
  }

  Future<List<RailCapability>> capabilities({
    String? country,
    String? currency,
    String platform = 'WEB',
    bool? deviceCapable,
  }) async {
    final response = await _get('/api/mobile/capabilities', {
      if (country != null) 'country': country,
      if (currency != null) 'currency': currency,
      'platform': platform,
      if (deviceCapable != null) 'deviceCapable': '$deviceCapable',
      'productType': 'FLIGHT',
    });
    final data = _json(response) as Map<String, dynamic>;
    return (data['rails'] as List? ?? const [])
        .whereType<Map>()
        .map((item) => RailCapability.fromJson(Map<String, dynamic>.from(item)))
        .toList();
  }

  Future<CheckoutPreview> prepareCheckout({
    required String offerId,
    required String preferredRail,
  }) async {
    final token = await _tokenProvider?.call();
    if (token == null || token.isEmpty) {
      throw PaygateApiException(
        'AUTH_REQUIRED',
        'Votre session doit etre actualisee avant la preparation du checkout.',
      );
    }
    final response = await _post(
      '/api/mobile/checkout/prepare',
      {'offerId': offerId, 'preferredPaymentRail': preferredRail},
      headers: {
  ...
  Future<http.Response> _get(String path, Map<String, String> query) =>
      _request(
        () => _client.get(_baseUri.replace(path: path, queryParameters: query)),
      );
  ...
  Future<http.Response> _request(
    ...
    try {
      ...
      throw PaygateApiException(
        data['code']?.toString() ?? 'SERVER_ERROR',
        data['error']?.toString() ?? 'Le service est indisponible.',
      );
    } on http.ClientException {
      throw PaygateApiException(
        'NO_CONNECTION',
        'Connexion indisponible. Verifiez votre reseau puis reessayez.',
      );
    } on TimeoutException {
      throw PaygateApiException(
        'TIMEOUT',
        'Le service a mis trop de temps a repondre.',
      );
    }
  }
